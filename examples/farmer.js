...
var sowEnabled = false
var reapEnabled = false
...
bot.on('chat', async (username, message) => {
  ...
  switch (true) {
    ...
    case /^craft \d+ \w+$/.test(message):
      ...
      break
    case /^sow (on|off)$/.test(message):
      sowEnabled = command[1] === 'on'
      break
    case /^reap (on|off)$/.test(message):
      reapEnabled = command[1] === 'on'
      break
  }
})
